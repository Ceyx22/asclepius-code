...
namespace dynamixel_hardware
{
  ...
  CallbackReturn DynamixelHardware::on_init(const hardware_interface::HardwareInfo &info)
  {
    ...
    //   return CallbackReturn::ERROR;
    // }
    // Do group syncwrite protocolkGoalVelocityIndex
    ...
    if (!dynamixel_workbench_.addSyncWriteHandler(
            ...
            &log))
    {
      RCLCPP_FATAL(rclcpp::get_logger(kDynamixelHardware), "8 %s", log);
      return CallbackReturn::ERROR;
    }
    ...
    uint16_t start_address = control_items_[kPresentPositionItem]->address;
    uint16_t read_length = control_items_[kPresentPositionItem]->data_length + 2;
      ...
      const rclcpp::Duration & /* period */)
  {
    ...
    if (!dynamixel_workbench_.syncRead(
            kGoalPositionIndex, &log))
    {
      RCLCPP_ERROR(rclcpp::get_logger(kDynamixelHardware), "2 %s", log);
    }
    ...
    {
      if (!dynamixel_workbench_.getSyncReadData(
              kGoalPositionIndex, &positions[i], &log))
      {
        RCLCPP_ERROR(rclcpp::get_logger(kDynamixelHardware), "1 %s", log);
      }
    }
  ...
  return_type DynamixelHardware::enable_torque(const bool enabled)
  {
    ...
    if (enabled && !torque_enabled_)
    {
      for (uint i = 0; i < info_.joints.size(); ++i)
      {
        ...
        {
          RCLCPP_FATAL(rclcpp::get_logger(kDynamixelHardware), "3 %s", log);
          return return_type::ERROR;
        }
      }
      reset_command();
    ...
    else if (!enabled && torque_enabled_)
    {
      for (uint i = 0; i < info_.joints.size(); ++i)
      {
        ...
        {
          RCLCPP_FATAL(rclcpp::get_logger(kDynamixelHardware), "4 %s", log);
          return return_type::ERROR;
        }
      }
  ...
  return_type DynamixelHardware::set_control_mode(const ControlMode &mode, const bool force_set)
  {
    ...
    if (mode == ControlMode::Position && (force_set || control_mode_ != ControlMode::Position))
    {
      ...
      for (uint i = 0; i < joint_ids_.size(); ++i)
      {
        ...
        {
          RCLCPP_FATAL(rclcpp::get_logger(kDynamixelHardware), "5 %s", log);
          return return_type::ERROR;
        }
      }
  ...
  CallbackReturn DynamixelHardware::set_joint_positions()
  {
    ...
    {
      RCLCPP_ERROR(rclcpp::get_logger(kDynamixelHardware), "7 %s", log);
    }
    return CallbackReturn::SUCCESS;
  }
