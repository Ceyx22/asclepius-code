...
int main(int argc, char **argv)
{
    ...
    robot_tree.getChain("base_link", "link4", chain);
    ...
    RCLCPP_INFO(rclcpp::get_logger("Trajectory"), "number of joints: %d", chain.getNrOfJoints());
    double total_time = 3.0;
    int trajectory_len = 100;
    ...
    for (int i = 0; i < trajectory_len; i++)
    {
        // set endpoint twist
        double t = i;
        twist.vel.x(cos(M_PI * t / trajectory_len));
        twist.vel.y(0); // sin(M_PI * t / trajectory_len)
